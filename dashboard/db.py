...
def lang_selector() -> str:
    ...
    with st.sidebar:
        ...
        st.page_link("pages/0_Overview.py", label=t("nav_overview"), icon=":material/bar_chart:")
        ...
        #
        # Роль читаємо з session_state, а НЕ через import auth — інакше
        # виходить кільце: сторінка імпортує auth, auth імпортує db,
        # db імпортує auth. Python на цьому падає ImportError.
        if st.session_state.get("auth_role", "admin") == "admin":
...
/* Кнопки ВСЕРЕДИНІ форм мають інший kind — без цього правила вони
   лишаються темними на світлій темі і текст не читається */
...
/* Чекбокси: рамка й галочка під тему */
[data-testid="stCheckbox"] label span {{
    border-color: {th["border"]} !important;
}}
[data-testid="stCheckbox"] label p {{ color: {th["text"]} !important; }}

/* Підказки-питання біля полів */
[data-testid="stTooltipIcon"] svg {{ fill: {th["muted"]} !important; }}
